...
public class dynamicTable091 : Agent
{
    [Header("UI")]
    ...
    [SerializeField] [Range(0,9)]private int mazeNumber = 2; // Default : 0 | Can be set for initialized trains.
    public int lim = 95; // Also set this for initialized trains.
    ...
    private Quaternion rotation => product.transform.localRotation;
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        if(!productClass.triggered){
            ...
            if(cube){
                float angleDifference = Quaternion.Angle(Quaternion.Euler(0f,0f,0f), product.transform.localRotation);
                rotationPoint = (-1) + ((angleDifference - 180)/(0 - 180)) * 2;
                if(rotationPoint<0.72f && rotationPoint > 0){rotationPoint += -1;}
            }
            ...
            if(directionPoint<0.7f && directionPoint>0){directionPoint += -1;}
            if(speed < 0.1f){speed = 0.1f;}
            if(closeness<0.1){closeness = 0.1f;}
            float reward_increase = 0;
            if(cube){reward_increase = rotationPoint * directionPoint * speed * 0.0002f;if(rotationPoint<0&&directionPoint<0){reward_increase*=-1.5f;}}else{reward_increase = directionPoint * speed * 0.0002f;}
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if(randomSize && CompletedEpisodes%1 == 0){
            product.transform.localScale = new Vector3(UnityEngine.Random.Range(4,9)*0.5f, UnityEngine.Random.Range(4,9)*0.5f, (UnityEngine.Random.Range(4,9))*0.5f);
        }
        if(maze){
            ...
            if(!changeMaze){
                if((CompletedEpisodes-lastCheckpoint)%checkpointCount == 0){
                    ...
                    if(winPercentage >= lim){
                        ...
                        else if(mazeNumber == 9){checkpointCount=25;lim=99;mazeChange = true;gameStates.Clear();}
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        RayCollect(); // RAY
        sensor.AddObservation(observation); // RAY     // 81th

        sensor.AddObservation(product.transform.eulerAngles);
        sensor.AddObservation(product.transform.localScale);       // 87th
    ...
    private void updateUI()
    {
        ...
        if (cube){
            ui.text += "\nRotation: "+product.transform.localRotation.eulerAngles+"\nRotation Point: "+rotationPoint;
        }
    }
