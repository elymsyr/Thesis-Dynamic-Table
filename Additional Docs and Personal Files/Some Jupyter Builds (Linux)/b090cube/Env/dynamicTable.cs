...
public class dynamicTable090 : Agent
{
    ...
    [Header("Set")]
    [SerializeField] private bool TestOld = true;
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        if(!productClass.triggered){
            ...
            if(cube){
                float angleDifference = Quaternion.Angle(Quaternion.Euler(0f,0f,0f), rotation);
                rotationPoint = (-1) + ((angleDifference - 180)/(0 - 180)) * 2;           
                if(rotationPoint<0.7f && rotationPoint > 0){rotationPoint += -1;}
            }
            ...
            if(cube){reward_increase = rotationPoint * directionPoint * speed * 0.0002f;}else{reward_increase = directionPoint * speed * 0.0002f;}
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        RayCollect(); // RAY
        sensor.AddObservation(observation); // RAY     // 61th
