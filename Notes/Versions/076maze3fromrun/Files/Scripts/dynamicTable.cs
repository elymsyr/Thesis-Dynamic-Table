...
public class dynamicTable076 : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        if (!onTarget){
            ...
            else if (table.gMaze && directionPoint > 1-difficulty && directionPoint < 0){directionPoint*=-1;} // New Row
    ...
    public void triggerReset(){
        difficulty = (float)Math.Pow(difficulty,multiplier) * difficulty * 0.886f;
        freq += 1;
        trigger++;
        lose_streak++;
    ...
    public void winReset(){
        win++;
        freq -= freq*0.1f;
        difficulty = (float)((float)Math.Pow(difficulty/multiplier,multiplier) * difficulty * 1.009);
